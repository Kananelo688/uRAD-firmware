...
def main():
	...
	parser.add_argument(
	    "-f", "--filename", 
	    type=str, 
	    help="Specifies the filename where the raw data is saved. If missing, the data is saved to 'outputIQ.txt'"
	)
	...
	parser.add_argument(
	    ...
	    help="Enable verbose output which means printing the progress to the console as the script runs."
	)
	...
	parser.add_argument('-c','--centre-frequency',type=int,
		required=False, 
		help='Specifies the carrier frequency for CW mode or start frequency of modes 2-4 of uRAD(see uRAD manual for more details). Must be an integer from 5-245 for mode 1 and 5-195 for other modes. If missing, it defaults to 5'
		)
	...
	parser.add_argument('-b','--bandwidth',type=int,required=False,
		...
		"and therefore ignored if given. If missing, it defaults to 240( maximum BW)",
		)

	parser.add_argument('-s','--samples-number',type=int,required=False,
		...
		'Must be values 50-200. If missing, it defaults to 200'
		)

	parser.add_argument('-t','--target-number',type = int, required=False,
		help = "Specifies the number of targets that the uRAD ca detect as the same time. Must be integer between 1 and 5. If missing, it defaults to 1"
		)

	parser.add_argument('-r','--maximum-range',type=int, 
		required=False,
		help="Specifies the maximum range for modes 2-4, and maximum velocity for mode 1 that uRAD can detect. If missing, it defaults to 1"
		)

	parser.add_argument('-i,--moving-target-indicator',type=int,required=False,

		help="Specifies whethere or not the moving target indicator should be activated( see manual for more details). Must be with 0 or 1. If missing, it defaults to 0"
		)
	...
	if verbose:
		print(f'Opening uRAD...', end = '', flush = True)
	# switch ON uRAD
